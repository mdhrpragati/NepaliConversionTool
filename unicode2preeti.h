 ...
 */
 /**
   ...
   *unicode2preeti.h->unicode2.h,preeti.h,converting_functions.h
...
/*
 *The above function carries out the conversion...but the conversion is not complete...The rest is done in reeti class
 */
std::vector<Pchar> unicode_Converter(std::string raw_data)
{
	 ...
	 */
	{
        ...
        else
        {
            ...
            {
				output+=db.return_OtherPreeti(5).return_Dev();
            }
			else if((var+i)==")")
			{
				output+=db.return_OtherPreeti(6).return_Dev();
			}
			else
			{
				output+=(var+i);
